...
def generate_cube_dimensions():
    x = np.random.uniform(0.1, 30)  # 2*x <= 60, so x is between 0.1 and 30
    y = np.random.uniform(0.1, 0.625)  # 2*y <= 1.25, so y is between 0.1 and 0.625
    z = x  # x = z
    return [round(x, 3), round(y, 3), round(z, 3)]

# Function to generate random dimensions for a cylinder
def generate_cylinder_dimensions():
    r = np.random.uniform(0.1, 0.3)  # r <= 0.3
    h = np.random.uniform(0.1, 0.625)  # 2*h <= 1.25, so h is between 0.1 and 0.625

...
class OnlyPosIK:
    ...
    def get_joint_angles_for_site(self, site_name):
        ...
        site_to_joint_mapping = {
            "tip1": [0, 1, 2, 3],
            "tip2": [4,5,6,7], # Example joint indices for tip1
            "tip3": [8, 9, 10, 11],
            "tip4": [12,13,14,15]  # Example joint indices for tip2 (tip3 in your XML)
        }
        joint_indices = site_to_joint_mapping.get(site_name, [])
        return self.sim.data.qpos[joint_indices]

if __name__ == "__main__":
    xml_path = "/home/linux/Documents/mujoco200_linux/leap hand ws/leap ik.xml"
    ik_solver = OnlyPosIK(xml_path)

    # Define target positions and site names for index finger and thumb
    goals = [
        np.array([0.025, 0.025, 0.03]),  # Goal for index fingertip
        np.array([-0.065, -0.025, 0.03])
            # Goal for thumb fingertip
    ]
    site_names = ["tip1","tip2", "tip3","tip4"]  # Sites for index finger and thumb
